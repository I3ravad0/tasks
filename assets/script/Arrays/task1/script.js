...
function getMostFrequent(json) {
    ...
    for (let days of json.temperature) {
        const accumulate = {};
        let maxFrequency = 0
        let mostFrequentElem;
        days.forEach(element => {
            accumulate[element] = (accumulate[element] || 0) + 1;

            if (accumulate[element] >= maxFrequency) {
                maxFrequency = accumulate[element]
                mostFrequentElem = element;
            }
        });
        frequentTemp.push(mostFrequentElem)
    }
    return frequentTemp;
}
